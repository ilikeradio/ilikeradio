...
if (command === 'stations') {
	...
	message.delete().catch(O_o=>{});
	// delete bots message
	message.channel.send(stationsEmbed).then(msg => {
        msg.delete(deleteafter)
        })
...
if (command === 'np') {
	...
	var song = artist + ' - ' + title //(obj.currentsong.song.artist_name ? obj.currentsong.song.artist_name : 'no artist name') + ' - ' + (obj.currentsong.song.title ? obj.currentsong.song.title : 'no title')
	...
	// delete invoking
	message.delete().catch(O_o=>{});
	// delete bot msg later
        message.channel.send(npEmbed).then(msg => {
	msg.delete(deleteafter)
	})
        });
}
